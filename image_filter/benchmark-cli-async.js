/**
 * Command-line Benchmark Runner (async version)
 * Run WASM vs JS benchmarks from the CLI with detailed logging
 * 
 * Usage:
 *   node benchmark-cli-async.js                 # Run all benchmarks
 *   node benchmark-cli-async.js --benchmark mandelbrot  # Run specific test
 *   node benchmark-cli-async.js --iterations 3  # Run each test 3 times
 *   node benchmark-cli-async.js --export results.json  # Export results
 */
...
// Load WASM module dynamically (ES module)
async function loadWasm() {
    try {
        ...
        console.log(`✓ Available functions: ${Object.keys(wasmModule).filter(k => !k.startsWith('_') && k !== 'default').slice(0, 10).join(', ')}...\n`);
        return wasmModule;
    } catch (e) {
        console.error('❌ Failed to load WASM module:');
        console.error('   Make sure to run: wasm-pack build --release --target web');
